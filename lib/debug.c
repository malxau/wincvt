...
VOID
DbgRealAssert( LPCSTR szCondition, LPCSTR szFunction, LPCSTR szFile, DWORD dwLine ) {
	TCHAR szTemp[2048];  // A stack trace can be big
	DWORD oldGLE;
...
#else
//
// We're not generating dbg support.  We still need to export this as a stub.
//
VOID
DbgRealAssert( LPCSTR szCondition, LPCSTR szFunction, LPCSTR szFile, DWORD dwLine )
{
